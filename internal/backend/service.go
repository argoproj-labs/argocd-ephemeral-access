package backend

import (
	"context"
	"fmt"

	api "github.com/argoproj-labs/ephemeral-access/api/ephemeral-access/v1alpha1"
	"github.com/argoproj-labs/ephemeral-access/pkg/log"
	apierrors "k8s.io/apimachinery/pkg/api/errors"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	"k8s.io/apimachinery/pkg/runtime"
	"k8s.io/apimachinery/pkg/runtime/schema"
	"k8s.io/client-go/dynamic"
)

const (
	resourceType = "accessrequests"
)

type Service interface {
	GetAccessRequest(ctx context.Context, name, namespace string) (*api.AccessRequest, error)
}

type K8sClient interface {
	GetAccessRequest(ctx context.Context, name, namespace string) (*api.AccessRequest, error)
}

type DefaultK8sClient struct {
	client *dynamic.DynamicClient
}

func (c *DefaultK8sClient) GetAccessRequest(ctx context.Context, name, namespace string) (*api.AccessRequest, error) {
	resource := schema.GroupVersionResource{
		Group:    api.GroupVersion.Group,
		Version:  api.GroupVersion.Version,
		Resource: resourceType,
	}
	resp, err := c.client.Resource(resource).Namespace(namespace).Get(ctx, name, metav1.GetOptions{})
	if err != nil {
		return nil, fmt.Errorf("error retrieving resource from k8s: %w", err)
	}
	u := resp.UnstructuredContent()
	var ar api.AccessRequest
	err = runtime.DefaultUnstructuredConverter.FromUnstructured(u, &ar)
	if err != nil {
		return nil, fmt.Errorf("error converting accessrequest unstructured: %w", err)
	}
	return &ar, nil
}

func NewK8sClient(c *dynamic.DynamicClient) *DefaultK8sClient {
	return &DefaultK8sClient{
		client: c,
	}
}

type DefaultService struct {
	client K8sClient
	logger log.Logger
}

func NewDefaultService(c K8sClient, l log.Logger) *DefaultService {
	return &DefaultService{
		client: c,
		logger: l,
	}
}

func (s *DefaultService) GetAccessRequest(ctx context.Context, name, namespace string) (*api.AccessRequest, error) {
	ar, err := s.client.GetAccessRequest(ctx, name, namespace)
	if err != nil {
		if apierrors.IsNotFound(err) {
			return nil, nil
		}
		return nil, fmt.Errorf("error getting accessrequest from k8s: %w", err)
	}
	return ar, nil
}
