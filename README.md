...
Releasing the project is just a matter of creating a tag following the
`v*` pattern and pushing this tag to the upstream repo. This task
needs to be executed by one of the project maintainers. Once the tag
is pushed the release workflow will be automatically triggered and a
release page will be published in github. The release notes is
generated by comparing the commits from the last tag and the current
one. The previous tag can be configured by setting the `GORELEASER_PREVIOUS_TAG`
variable as part of the github repo settings:
...
When we want to create a final release after a series of release
candidates and have a consolidated release notes it is necessary to
manually specify the previous tag to compare. The previous tag can be
specified by setting the `GORELEASER_PREVIOUS_TAG` variable in the
